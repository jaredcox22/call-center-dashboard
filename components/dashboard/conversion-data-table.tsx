...
export function ConversionDataTable({ 
  ...
  const getRecordId = (record: ConversionRecord): string => {
  ...
  const summaryStats = useMemo(() => {
    ...
    const totalPitched = nonExcludedData.length
    const totalConversions = nonExcludedData.filter((record) => record.positive === 1).length
    const conversionRate = totalPitched > 0 ? Math.round((totalConversions / totalPitched) * 100) : 0
    const totalQualified = nonExcludedData.filter((record) => record.qualified === true).length
    const totalUnqualified = nonExcludedData.filter((record) => record.qualified === false && record.positive === 1).length
