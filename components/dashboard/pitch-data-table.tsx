  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    filtered = filtered.filter((record) => {
      ...
      return (
        ...
        (record.pitched === 1 ? "yes" : "no").includes(query)
      )
    })
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-[95vw] lg:max-w-7xl max-h-[90vh] overflow-hidden flex flex-col p-0 gap-0">
        <DialogHeader className="px-6 pt-6 pb-4">
          ...
          <DialogDescription>
            View and search all connected calls with their pitch status. Click column headers to sort.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4 flex-1 min-h-0 px-6 pb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <Card className="p-3">
              ...
              <div className="text-lg font-semibold">{summaryStats.totalConnected}</div>
            </Card>
            <Card className="p-3">
              <div className="text-xs text-muted-foreground mb-1">Total Pitched</div>
              <div className="text-lg font-semibold text-green-600 dark:text-green-400">{summaryStats.totalPitched}</div>
            </Card>
            <Card className="p-3">
              <div className="text-xs text-muted-foreground mb-1">Pitch Rate</div>
              <div className="text-lg font-semibold">{summaryStats.pitchRate}%</div>
            </Card>
          </div>
                ...
                <CollapsibleContent>
                  <AlertDescription>
                    <div className="space-y-2 pt-2">
                      <p>
                        <strong>What's included:</strong> This table shows all calls where a connection was made (connected = 1) within the selected date range and filters. 
                        The pitch rate is calculated as: (Total Pitched ÷ Total Connected) × 100.
                      </p>
                      <p>
                        <strong>Pitch Status:</strong> A call is considered pitched when pitched = 1. This indicates whether a pitch was made during the connected call.
                      </p>
                    </div>
                  </AlertDescription>
                </CollapsibleContent>
