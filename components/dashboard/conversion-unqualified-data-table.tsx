...
export function ConversionUnqualifiedDataTable({ 
  ...
  const getRecordId = (record: ConversionUnqualifiedRecord): string => {
    return record.id?.toString() ?? ""
  }

  // Filter to all unqualified calls (qualified === false) and not excluded
  const unqualifiedData = useMemo(() => {
    return data.filter((record) => {
      const recordId = getRecordId(record)
      return record.qualified === false && recordId && !excludedIds.has(recordId)
    })
  }, [data, excludedIds])
  ...
  }, [data, unqualifiedData, excludedIds])
